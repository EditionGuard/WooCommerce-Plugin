...
if (($_SERVER["SCRIPT_NAME"]==$post_url)&&$_GET["post"])
	...
	if ($post->post_type=="product") $show_edition_guard=true;
	}
elseif (($_SERVER["SCRIPT_NAME"]==$post_new_url)&&($_GET["post_type"]=="product")) $show_edition_guard=true;
else $show_edition_guard=false;
...
if ($show_edition_guard)
{
	...
	$email = get_option('woo_eg_email'); if ($email=="") $email="";
	if ($secret) $hash = hash_hmac("sha1", $nonce.$email, base64_decode($secret)); else $hash="";
	...
	if (($email!="")&&($hash!=""))
		...
		$library=json_decode(woo_eg_curl_it("http://www.editionguard.com/api/ebook_list",$data));
		}
	else $library="";
...
function woo_eg_options()
	...
	if ($_SESSION["woo_eg_options_updated"])
		...
		?><div id="setting-error-settings_updated" class="updated settings-error"><p><strong>Settings saved.</strong></p></div><?php
		if ($_REQUEST["return_url"]) {?><script>jQuery(document).ready(function(){
...
function woo_eg_curl_it($uri, $data, $method = 'POST')
	{
	if($method=='GET') $uri .= (!empty($data)) ? "?$data" : "";
	$ch = curl_init();
