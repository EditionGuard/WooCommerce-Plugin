jQuery(document).ready(function () {
    ...
    if (woo_eg.library)
        jQuery.each(woo_eg.library, function (k, v) {
...
function use_ebook(object = null, isVariation = false)
{
    if (isVariation) {
        let variation = jQuery(object).parents('.woocommerce_variation');
        let resource_id = variation.find('.ebook_library').val();
        variation.find('._eg_resource_id').val(resource_id);
        console.log(variation.find('.ebook_library option:selected'));
        variation.find('._current_ebook').html(variation.find('.ebook_library option:selected').text());
        variation.find('._eg_title').val(variation.find('.ebook_library option:selected').attr('title'));
        variation.find('._eg_drm_type').val(variation.find('.ebook_library option:selected').attr('data-drm-type'));
    } else {
...
function use_editionguard_drm_trigger(object, isVariation = false)
{
    if (object.checked)
    {
        ...
        if (isVariation) {
            let variation = jQuery(object).parents('.woocommerce_variation');
            variation.find('.show_if_variation_downloadable .form-field').hide();
            variation.find('.show_if_variation_downloadable .downloadable_files').hide();
            variation.find('.show_if_variation_downloadable .form-field-drm').show();
            variation.find('.show_if_variation_downloadable .e-book').show();
            variation.find('.show_if_variation_downloadable .use_button_edition_guard').show();
            variation.find('.custom-combobox').show();
        } else {
            jQuery('.options_group.show_if_downloadable .form-field').hide();
            jQuery('.options_group.show_if_downloadable .form-field-drm').show();
            jQuery('.options_group.show_if_downloadable .e-book').show();
        ...
        if (isVariation) {
            let variation = jQuery(object).parents('.woocommerce_variation');
            variation.find('.show_if_variation_downloadable .form-field').show();
            variation.find('.show_if_variation_downloadable .downloadable_files').show();
            variation.find('.show_if_variation_downloadable .form-field-drm').hide();
            variation.find('.show_if_variation_downloadable .e-book').hide();
            variation.find('.show_if_variation_downloadable .use_button_edition_guard').hide();
            variation.find('.custom-combobox').hide();
        } else {
